...
echo "$(getent ahostsv4 host.docker.internal | awk '{ print $1 }') fake.localmaeher.pvarki.fi" >>/etc/hosts
echo "$(getent ahostsv4 host.docker.internal | awk '{ print $1 }') tak.localmaeher.pvarki.fi" >>/etc/hosts
echo "$(getent ahostsv4 host.docker.internal | awk '{ print $1 }') kc.localmaeher.pvarki.fi" >>/etc/hosts
