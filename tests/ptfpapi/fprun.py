...
async def handle_fragment(request: web.Request) -> web.Response:
    ...
    LOGGER.info(f"Called with user={user}")
...
async def handle_description_v2(request: web.Request) -> web.Response:
    ...
    _lang = request.match_info.get("language", "en")
    LOGGER.info(f"Called with language={_lang}")
...
async def handle_admin_clients_data_v2(request: web.Request) -> web.Response:
    ...
    check_peer_cert(request)
    payload = await request.json()
    LOGGER.info(f"Called admin with payload={payload}")
...
def main() -> int:
    ...
    LOGGER.info(f"Starting runner on port {bind_port}")
    web.run_app(app, host=bind_address, port=int(bind_port), ssl_context=ssl_ctx)
    return 0
