...
async def test_enroll_show_verif_code(tilauspalvelu_jwt_admin_client: TestClient) -> None:
    ...
    resp_dict: Dict[Any, Any] = resp.json()
    LOGGER.debug(resp_dict)
    code: str = resp_dict["verification_code"]

    resp = await tilauspalvelu_jwt_admin_client.get(
        f"/api/v1/enrollment/show-verification-code-info?verification_code={code}"
    )
    resp_dict = resp.json()
    LOGGER.debug(resp_dict)
    ...
    resp_dict = resp.json()
    LOGGER.debug(resp_dict)
    code = resp_dict["verification_code"]

    resp = await tilauspalvelu_jwt_admin_client.get(
        f"/api/v2/enrollment/show-verification-code-info?verification_code={code}"
    )
    resp_dict = resp.json()
    LOGGER.debug(resp_dict)
...
async def test_post_init(tilauspalvelu_jwt_admin_client: TestClient) -> None:
    ...
    # V2
    json_dict = {"callsign": "superjuusto"}
...
async def test_accept_no_such_user(tilauspalvelu_jwt_admin_client: TestClient) -> None:
    ...
    # V2
    json_dict = {"approvecode": "nosuchcode"}
    callsign = "duhnosuchuser"
    resp = await tilauspalvelu_jwt_admin_client.post(f"/api/v2/enrollment/{callsign}/approve", json=json_dict)
    resp_dict = resp.json()
    LOGGER.debug(resp_dict)
    assert resp.status_code == 404
...
async def test_invitecode_create(tilauspalvelu_jwt_admin_client: TestClient) -> None:
    ...
    resp_dict = resp.json()
    assert resp.status_code == 200
    assert inv_code != resp_dict["invite_code"]
...
async def test_invitecode_dectivate(tilauspalvelu_jwt_admin_client: TestClient) -> None:
    ...
    json_dict: Dict[Any, Any] = {"invite_code": inv_code}
    ...
    json_dict = {"invite_code": inv_code}
    resp = await tilauspalvelu_jwt_admin_client.put("/api/v1/enrollment/invitecode/deactivate", json=json_dict)
    resp_dict = resp.json()
    LOGGER.debug(resp_dict)
    assert resp.status_code == 200
...
async def test_invite_code(tilauspalvelu_jwt_admin_client: TestClient) -> None:
    ...
    resp = await tilauspalvelu_jwt_admin_client.get(f"/api/v1/enrollment/invitecode?invitecode={inv_code}")
    resp_dict = resp.json()
    LOGGER.debug(resp_dict)
...
async def test_enroll_with_invite_code(  # pylint: disable=R0915
    ...
    json_dict: Dict[Any, Any] = {"invite_code": inv_code, "callsign": "enrollenrique"}
    ...
    json_dict = {"invite_code": inv_code, "callsign": "enrollenrique"}
    resp = await unauth_client.post("/api/v1/enrollment/invitecode/enroll", json=json_dict)
    resp_dict = resp.json()
    LOGGER.debug(resp_dict)
    assert resp.status_code == 400
    ...
    json_dict = {"invite_code": inv_code, "callsign": "enriquescousin"}
    resp = await unauth_client.post("/api/v1/enrollment/invitecode/enroll", json=json_dict)
    resp_dict = resp.json()
    LOGGER.debug(resp_dict)
    assert resp.status_code == 400
    ...
    json_dict = {"invite_code": inv_code, "callsign": "enrollenriqueV2"}
    resp = await unauth_client.post("/api/v2/enrollment", json=json_dict)
    resp_dict = resp.json()
    LOGGER.debug(resp_dict)
    assert resp.status_code == 200
    assert resp_dict["jwt"] != ""
    ...
    # list enrollments filter by code
    resp = await tilauspalvelu_jwt_admin_client.get(f"/api/v2/enrollment/{enrique_ac}")
    resp_dict = resp.json()
    assert resp_dict["callsign_list"] is not None
    assert resp_dict["callsign_list"][0]["callsign"] == "enrollenriqueV2"
    assert resp_dict["callsign_list"][0]["approvecode"] == enrique_ac
    ...
    # ENROLL WITH INVITE CODE - CODE IS LOCKED
    resp = await tilauspalvelu_jwt_admin_client.put(f"/api/v2/invite-code/{inv_code}/deactivate")
    resp_dict = resp.json()
    LOGGER.debug(resp_dict)
    assert resp.status_code == 200

    json_dict = {"invite_code": inv_code, "callsign": "enriquescousinV2"}
    resp = await unauth_client.post("/api/v2/enrollment", json=json_dict)
    resp_dict = resp.json()
    LOGGER.debug(resp_dict)
    assert resp.status_code == 400
    ...
    # Accept the enrollment
    json_dict = {"approvecode": enrique_ac}
    callsign: str = "enrollenriqueV2"
    resp = await tilauspalvelu_jwt_admin_client.post(f"/api/v2/enrollment/{callsign}/approve", json=json_dict)
    resp_dict = resp.json()
    LOGGER.debug(resp_dict)
    assert resp.status_code == 200
    ...
    # Fetch the PFX
    unauth_client.headers.update({"Authorization": f"Bearer {enrique_jwt}"})
    resp = await unauth_client.get("/api/v2/enduserpfx/enrollenriqueV2")
    resp.raise_for_status()
    pfxdata = cryptography.hazmat.primitives.serialization.pkcs12.load_pkcs12(resp.content, b"enrollenriqueV2")
    assert pfxdata.key
    assert pfxdata.cert
    del unauth_client.headers["Authorization"]
